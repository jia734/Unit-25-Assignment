...
Public Class Form4
    Public Shared AttendanceRecords As New Dictionary(Of String,
    Dictionary(Of DateTime, Dictionary(Of String, String)))()
...
ComboBox2.SelectedItem?.ToString()
        ...
        For Each studentID As String In Form2.Students.Keys
            ...
            Dim studentTeachers As List(Of String) =
studentData("Teacher")
            Dim studentUnits As String = String.Join(", ",
studentData("Units"))

            If studentLevel = selectedLevel AndAlso
studentTeachers.Contains(selectedTeacher) Then
                DataGridView1.Rows.Add(studentName, studentID,
studentLevel, studentUnits, False)
            End If
        Next
    End Sub
...
row.Cells("StudentID").Value.ToString()
            ...
            If Not AttendanceRecords.ContainsKey(studentID) Then
                AttendanceRecords(studentID) = New Dictionary(Of
DateTime, Dictionary(Of String, String))()
            End If

            If Not 
AttendanceRecords(studentID).ContainsKey(currentDate) Then 
                AttendanceRecords(studentID)(currentDate) = New 
Dictionary(Of String, String)()
            End If

            Dim units As List(Of String) =
Form2.Students(studentID)("Units")
            For Each unit As String In units
                AttendanceRecords(studentID)(currentDate)(unit) =
attendanceStatus
            Next
        Next
